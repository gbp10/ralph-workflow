...
Save the PRD to: .kiro/specs/$FEATURE_NAME/requirements.md
...
When complete, output: <promise>PRD COMPLETE</promise>"
    ...
    if [[ -f "$PROJECT_DIR/.kiro/specs/$FEATURE_NAME/requirements.md" ]]; then
...
Save outputs to:
- Research files: .kiro/specs/$FEATURE_NAME/research/
- Synthesis: .kiro/specs/$FEATURE_NAME/research-synthesis.md
- Blueprint: .kiro/specs/$FEATURE_NAME/implementation-blueprint.md
...
When complete, output: <promise>BLUEPRINT COMPLETE</promise>"
    ...
    if [[ -f "$PROJECT_DIR/.kiro/specs/$FEATURE_NAME/implementation-blueprint.md" ]]; then
...
Save to: .claude/ralph-workflow/stories/$FEATURE_NAME.json
...
When complete, output: <promise>STORIES COMPLETE</promise>"
    ...
    if [[ -f "$PROJECT_DIR/.claude/ralph-workflow/stories/$FEATURE_NAME.json" ]]; then
        log "${GREEN}✓ Phase 3 Complete: Stories generated${NC}"
        # Validate JSON
        if jq . "$PROJECT_DIR/.claude/ralph-workflow/stories/$FEATURE_NAME.json" > /dev/null 2>&1; then
            local story_count=$(jq '.user_stories | length' "$PROJECT_DIR/.claude/ralph-workflow/stories/$FEATURE_NAME.json")
...
run_phase_4_execute() {
    ...
    local stories_file="$PROJECT_DIR/.claude/ralph-workflow/stories/$FEATURE_NAME.json"
...
summary() {
    ...
    log "  📄 PRD:       .kiro/specs/$FEATURE_NAME/requirements.md"
    log "  🔬 Research:  .kiro/specs/$FEATURE_NAME/research/"
    log "  📋 Blueprint: .kiro/specs/$FEATURE_NAME/implementation-blueprint.md"
    log "  📝 Stories:   .claude/ralph-workflow/stories/$FEATURE_NAME.json"
    log ""
